...
- (void) createSectionsForContacts: (NSArray*) contacts {
    for(SinaFriend *contact in contacts) {
        
        NSString *sectionLetter = [self sectionLetterForContact: contact.name];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *friendListTableViewCellIdentifier = @"FriendViewCell";
    ...
    FriendViewCell *cell = [tableView dequeueReusableCellWithIdentifier:friendListTableViewCellIdentifier];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:friendListTableViewCellIdentifier owner:self options:nil] objectAtIndex:0];
    }
    
    SinaFriend * contact = ((SinaFriend*)[contacts objectAtIndex:[indexPath row]]);
    
    cell.name.text = contact.name;
    
    [cell.avatar setImageWithURL:[NSURL URLWithString:contact.profile_image]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SinaFriend* contact = [contacts objectAtIndex: indexPath.row];
    [self.delegate didFinishContactSelectionWithContacts:contact];
}
...
- (void)filterContentForSearchText:(NSString*)searchText scope:(NSString*)scope
{
    ...
    for (id key in self.sections)
    {
        ...
        for (SinaFriend *contact in section)
        {
